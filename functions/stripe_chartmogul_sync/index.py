...
def upload_plan(invoice, source_id, platform):
    ...
    if platform == "stripe":
        ...
        interval_count = invoice["data"][0]["lines"]["data"][0]["plan"][
            "interval_count"
        ]
...
def upload_customer(customer, source_id, platform):
    ...
    if platform == "stripe":
        ...
        plan_start = customer["subscriptions"]["data"][0]["items"]["data"][0]["plan"][
            "created"
        ]
        plan_start = datetime.datetime.fromtimestamp(int(plan_start)).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
        trial_start = customer["subscriptions"]["data"][0]["trial_start"]

    if trial_start != None:
        trial_start = datetime.datetime.fromtimestamp(int(trial_start)).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
...
def upload_invoice(invoice, cm_customer_id, cm_plan_id, cm_source_id, platform):
    ...
    if platform == "stripe":
        ...
        for invoice in stripe_invoices["data"]:
            ...
            if invoice["status"] == "paid":
                ...
                trans_type = "refund"
            if invoice["status"] == "paid":
                trans_result = "successful"
            else:
                trans_result = "failed"

            xx = {
                "external_id": invoice["id"],
                "date": datetime.datetime.fromtimestamp(int(invoice_date)).strftime(
                    "%Y-%m-%d %H:%M:%S"
                ),
                "currency": invoice["currency"].upper(),
                "due_date": datetime.datetime.fromtimestamp(int(due_date)).strftime(
                    "%Y-%m-%d %H:%M:%S"
                ),
                ...
                "line_items": [
                    {
                        ...
                        "service_period_start": datetime.datetime.fromtimestamp(
                            int(period_start)
                        ).strftime("%Y-%m-%d %H:%M:%S"),
                        "service_period_end": datetime.datetime.fromtimestamp(
                            int(period_end)
                        ).strftime("%Y-%m-%d %H:%M:%S"),
                        "amount_in_cents": int(
                            invoice["lines"]["data"][0]["plan"]["amount_decimal"]
                        ),
                        "quantity": invoice["lines"]["data"][0]["quantity"],
                    }
                ],
                "transactions": [
                    {"date": trans_date, "type": trans_type, "result": trans_result}
                ],
            }
    ...
    res = chartmogul.Invoice.create(
        config, uuid=cm_customer_id, data={"invoices": [xx]}
    )
...
def update_invoices(event, source_id):
    if event["detail-type"] in (
        ...
        if cm_customer.entries[0].uuid != None:
            ...
            cm_inv = rq.get(
                inv_url, auth=(chartmogul_api_token, chartmogul_api_key)
            ).json()
            ...
            for invoice in stripe_invoices:
                for plan_id in str_plan_ids:
                    ...
                    upload_invoice(
                        invoice, cm_cus_uuid, cm_plan_id, source_id, platform="stripe"
                    )

        # create new customer and upload all invoices if they do not already exist
        else:
            customer = upload_customer(
                event, "ds_c52fc4d6-f627-11eb-9764-33db032012f1", platform="stripe"
            )
            for invoice in stripe_invoices:
                for plan_id in str_plan_ids:
                    cm_plan_id = get_plan_id(plan_id)
                    upload_invoice(
                        invoice, customer, cm_plan_id, source_id, platform="stripe"
                    )
